...
public class ImageClassifier {
    ...
    public static double[] extractFeatures(Picture picture) {
        ...
        for (int row = 0; row < height; row++) {
            for (int col = 0; col < width; col++) {
                ...
                int gray = color
                        .getRed();//since images are grayscale, can get R,G, or B (all are equal).
    ...
    public static void main(String[] args) {
        ...
        while (!training.isEmpty()) {
            ...
            double[] x = extractFeatures(
                    picture); //puts image into 1D array vectors representing RGB vals

            object.trainMulti(x,
                              label); //trains perceptron based on array, and what image the array should represent
        ...
        while (!testing.isEmpty()) {
            ...
            double[] x = extractFeatures(
                    picture); //puts image into 1 array vectors representing RGB vals

            int prediction = object
                    .predictMulti(x); //will predict the label based on fed 1D array vectors

            if (prediction != label) {
                errors++;
                StdOut.println(filename + " label: " + label + " predicted: " + prediction);
            }
            total++;
